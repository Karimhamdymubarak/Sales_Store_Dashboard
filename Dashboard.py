...
with col1:
    ...
    fig = px.histogram(data_frame=df , y ="Sales" , x ="Category" , histfunc='sum',width=450 ,text_auto=True).update_xaxes(categoryorder="total descending")
    st.plotly_chart(fig)
    
with col2:
    st.header("Sales By Year")
    fig_2 = px.histogram(data_frame=df , x ="Year" , y ="Sales" , histfunc='sum' ,width=550,text_auto=True).update_xaxes(categoryorder="total descending")
